...
module PATTERN (
    ...
    output logic signed [2303:0] weight, // 256 * 16 = 4096 bit
    ...
    initial begin
        ...
        weight_file = $fopen("files/fc1_weights_hex.txt", "r");
    ...
    // =================================================================
    initial begin
        golden_file = $fopen("files/tv_1_fc1_out_hex.txt", "r");
        if (golden_file == 0) begin
            $display("錯誤：無法開啟 golden_file 'tv_1_fc1_out_hex.txt'");
            $finish;
        end
    end
    ...
    always @(negedge clk) begin
        if (rst_n == 1'b1) begin 
           ...
           //if(test_count>=1024)begin
            ...
            // 2. 檢查 DUT 是否有有效輸出
            if (out_valid && reset_cycle_count>=90801) begin
                if (golden_file_is_done) begin
