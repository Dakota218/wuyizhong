...
module PATTERN (
    ...
    output logic signed [2047:0] weight, // 256 * 16 = 4096 bit
    ...
    // 參數與內部變數
    // =================================================================
    parameter NUM_WEIGHTS_PER_CYCLE = 128;
    parameter WEIGHT_PATTERN_CYCLES = 64;
    parameter TOTAL_RAM_SIZE = NUM_WEIGHTS_PER_CYCLE * WEIGHT_PATTERN_CYCLES; // 16384
    ...
    initial begin
        ...
        weight_file = $fopen("files/attn_kv_w_hex.txt", "r");
        ...
        // (C) 開啟 'in_data' 檔案
        input_file = $fopen("files/tv_attn_3_sr_ln_out_hex.txt", "r");
        if (input_file == 0) begin $display("錯誤：無法開啟 tv_attn_3_sr_ln_out_hex.txt"); $finish; end
        ...
        begin
            ...
            while (!$feof(input_file)) begin
                ...
                while (active_cycle_count < 64 && !$feof(input_file)) begin
                    ...
                    for (j = 0; j < NUM_WEIGHTS_PER_CYCLE; j = j + 1) begin
                        ram_index = (j * WEIGHT_PATTERN_CYCLES) + cycle_offset;
                ...
                in_valid     <= 1'b0;
                in_data  <= 0;
                for (pause_cycle_count = 0; pause_cycle_count <= 173; pause_cycle_count = pause_cycle_count + 1) begin
                    @(negedge clk);
                end
            ...
            // (F) 檔案讀取完畢，清理
            $display("[%0t] PATTERN: 輸入檔案 'tv_attn_3_sr_ln_out_hex.txt' 讀取完畢。", $time);
            in_valid     <= 1'b0;
            //w_load_valid <= 1'b0;
            $fclose(input_file);
        end
    ...
    // =================================================================
    initial begin
        golden_file = $fopen("files/tv_attn_4_kv_out_hex.txt", "r");
        if (golden_file == 0) begin
            $display("錯誤：無法開啟 golden_file 'tv_attn_4_kv_out_hex.txt'");
            $finish;
        end
    end
