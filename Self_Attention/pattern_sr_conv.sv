...
module PATTERN (
    ...
    output logic signed [575:0] weight, // 64 * 16 = 1024 bit
    ...
    // 參數與內部變數
    // =================================================================
    parameter NUM_WEIGHTS_PER_CYCLE = 64;
    parameter WEIGHT_PATTERN_CYCLES = 64;
    parameter TOTAL_RAM_SIZE = NUM_WEIGHTS_PER_CYCLE * WEIGHT_PATTERN_CYCLES * 64; 

    // 權重檔案的預載入記憶體
    reg signed [20:0] tb_full_weight_ram [0:TOTAL_RAM_SIZE-1];
    ...
    initial begin
        ...
        weight_file = $fopen("files/attn_sr_w_hex.txt", "r");
        ...
        // (C) 開啟 'in_data' 檔案
        input_file = $fopen("files/tv_attn_0_input_hex.txt", "r");
        if (input_file == 0) begin $display("錯誤：無法開啟 tv_attn_0_input_hex.txt"); $finish; end
        ...
        begin
            ...
            //integer active_cycle_count;
            integer kernel_offset = 0;
            integer col_count_8 = 0;
            $display("階段 2: 啟動激勵循環...");
            ...
            while (!$feof(input_file)) begin
                ...
                //while (active_cycle_count < 64 && !$feof(input_file)) begin
                    ...
                    for (j = 0; j < NUM_WEIGHTS_PER_CYCLE; j = j + 1) begin
                        ram_index = (j * WEIGHT_PATTERN_CYCLES*64) + cycle_offset*64 + kernel_offset;
                        weight[ (j*9) +: 9 ] = tb_full_weight_ram[ram_index];
                    end
                    ...
                    if (cycle_offset == WEIGHT_PATTERN_CYCLES) begin
                        ...
                        if(kernel_offset % 8 == 7)begin
                            if(col_count_8 == 21)begin
                                if(kernel_offset == 63)begin
                                    kernel_offset = 0;
                                end else begin
                                    kernel_offset = kernel_offset + 1;
                                end
                            end else begin
                                kernel_offset = kernel_offset - 7;
                            end
                            col_count_8 = (col_count_8 + 1)%22;
                        end else begin
                            kernel_offset = kernel_offset + 1;
                        end
                    end
                ...
                in_valid     <= 1'b0;
                in_data  <= 0;
                /*for (pause_cycle_count = 0; pause_cycle_count <= 3; pause_cycle_count = pause_cycle_count + 1) begin
                    @(negedge clk);
                end*/
            end 

            // (F) 檔案讀取完畢，清理
            $display("[%0t] PATTERN: 輸入檔案 'tv_attn_0_input_hex.txt' 讀取完畢。", $time);
            in_valid     <= 1'b0;
            $fclose(input_file);
        end
    end 
    ...
    // =================================================================
    initial begin
        golden_file = $fopen("files/tv_attn_2_sr_conv_out_pixel_major_hex.txt", "r");
        if (golden_file == 0) begin
            $display("錯誤：無法開啟 golden_file 'tv_attn_2_sr_conv_out_pixel_major_hex.txt'");
            $finish;
        end
    end
