...
module PATTERN (
    ...
    output logic signed [10:0] in_data,
    output logic signed [511:0] weight, // 64 * 16 = 1024 bit
    ...
    parameter WEIGHT_PATTERN_CYCLES = 256;
    ...
    initial begin
        ...
        weight_file = $fopen("files/fc2_weights_hex.txt", "r");
        ...
        // (C) 開啟 'in_data' 檔案
        input_file = $fopen("files/tv_3_gelu_out_hex.txt", "r");
        if (input_file == 0) begin $display("錯誤：無法開啟 tv_3_gelu_out_hex.txt"); $finish; end
            ...
            while (!$feof(input_file)) begin
                ...
                // --- 階段 A: Active (傳送 64 週期) ---
                    ...
                    for (j = 0; j < NUM_WEIGHTS_PER_CYCLE; j = j + 1) begin
                        ...
                        weight[ (j*8) +: 8 ] = tb_full_weight_ram[ram_index];
                    end
                    ...
                    in_valid <= 1'b0;
                    in_data  <= 0;
                    for (pause_cycle_count = 0; pause_cycle_count <= 8; pause_cycle_count = pause_cycle_count + 1) begin
                        @(negedge clk);
                    end
