...
`include "pattern_fc2.sv"

// 2. 根據模擬類型 (RTL vs. GATE) 包含 DUT 原始碼
`ifdef RTL
    `define DUMP_FILE "fc2_rtl.fsdb"
    `define SDF_INSTANCE I_DUT
    `define SDF_FILE "fc2_syn.sdf"
    `include "fc2.sv"
    `include "fc2_column.sv"
    //`include "multiply.sv"
    //`include "csa_3_to_2.sv"
    //`include "full_adder.sv"
    //`include "booth_encoder.sv"
    
`elsif GATE
    `define DUMP_FILE "fc2_gate.fsdb"
    `define SDF_FILE "fc2_syn.sdf"
    `define SDF_INSTANCE I_DUT
...
module TESTBED();
    ...
    logic             in_valid;
    logic signed [10:0] in_data;
    logic signed [511:0] weight;
