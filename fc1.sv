...
logic [7:0] out_phase_count,out_phase_count_n;
logic [3:0] cycle_count, cycle_count_n; 
...
always_comb begin
    //state_n  = state;
    case (state)
        IDLE:  state_n = (reset_ram_valid)?        RESET_RAM : state;
        RESET_RAM:  state_n = (in_valid)?        INPUT : state;
        INPUT: state_n = (in_count == 67)?  OUTPUT1 : state;
        OUTPUT1: state_n = (out_phase_count == 177)? OUTPUT2 : state;
        OUTPUT2: state_n = state;
...
always_comb begin
    ...
    cycle_count_n = cycle_count;
    out_phase_count_n = out_phase_count;
    start_in_n = start_in;
    ...
    case (state)
        ...
        OUTPUT1: begin
            ...
            if(out_count == 254)begin
                out_phase_count_n = out_phase_count + 1;
            end
            if (out_count == 255) begin
                out_count_n = 0;
            end else if (out_count >= 188) begin
                in_count_n = in_count + 1;
                for (int j=0;j<256;j=j+1) begin
                    count_n[j]=count[j]+1;
                end
        ...
        OUTPUT2: begin
            ...
            if(out_count == 255 && cycle_count == 8)begin
                start_in_n = 0;
            end else if(out_count >= 249 && cycle_count == 1)begin
                start_in_n = 1;
            end 
        end
    endcase
...
always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        out_phase_count <= out_phase_count_n;
